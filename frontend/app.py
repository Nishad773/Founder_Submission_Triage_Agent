...
ANALYZE_URL = f"{BACKEND_API_URL.rstrip('/')}/analyze"
...
def _build_request_files(upload_map: dict[str, list]) -> list[tuple[str, tuple[str, bytes, str]]]:
    files: list[tuple[str, tuple[str, bytes, str]]] = []
    for field_name, uploads in upload_map.items():
        for upload in uploads:
            mime_type = "application/pdf" if upload.name.lower().endswith(".pdf") else "application/vnd.openxmlformats-officedocument.presentationml.presentation"
            files.append((field_name, (upload.name, upload.getvalue(), mime_type)))
    return files
...
if st.button("Analyze", type="primary", use_container_width=True, disabled=not any(uploads.values())):
    try:
        with st.spinner("Analyzing founder materials with Gemini..."):
            response = requests.post(ANALYZE_URL, files=_build_request_files(uploads), timeout=180)
